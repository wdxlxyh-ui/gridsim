...
微电网仿真模块提供图形化组态编辑和实时仿真能力，支持光伏、储能、负荷、充电桩四种设备类型，通过 IEC104 测点体系与标准规约无缝对接。

### 核心功能

| 功能 | 说明 |
|------|------|
| 拓扑组态编辑 | 可视化配置关口表/母线/设备/开关 |
| 实时仿真引擎 | 6-step tick 循环（PV→负荷→储能→功率平衡→store同步→公式求值） |
| 功率平衡公式 | GRID = Load + Charger + Battery - PV（自动生成） |
...
```
web/MicrogridEditor.vue           ← 前端组态编辑器
internal/microgrid/
