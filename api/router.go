...
func (h *Handler) handleStatus(w http.ResponseWriter, r *http.Request) {
	...
	status := map[string]interface{}{
		...
		"total_points":      h.store.TotalCount(),
		"uptime_seconds":    h.status.Uptime(),
		"total_interrogations": interrog,
		"total_controls":       control,
		"total_spontaneous":    spont,
		"point_counts": map[string]int{
			"AI": counts[config.TypeAI],
			"DI": counts[config.TypeDI],
			"PI": counts[config.TypePI],
			"DO": counts[config.TypeDO],
			"AO": counts[config.TypeAO],
		},
	}
