...
GridSim 是一个 IEC104/Modbus TCP 双协议从站模拟器，支持多实例并行运行、微电网仿真、11种自动变化策略，并提供 REST API 和 MCP Server 两种控制接口。**v3.1.0 新增操作引导、Dashboard 仪表盘、CommandPalette 命令面板、PropertyPanel 属性面板等 UX 改进。**
...
| 页面 | 路由 | 功能 |
|------|------|------|
| 仪表盘 | `/dashboard` | 运行总览、统计卡片、规约分布、快捷操作 (v3.1) |
...
- **操作引导** — 点击右下角 ❓ FAB 按钮，选择基础/高级引导，跨页面导航 (v3.1)
