...
func NewInstanceManagerServer(client *SimulatorClient) *server.MCPServer {
	...
	s.AddTool(mcp.NewTool("create_instance",
		...
		mcp.WithString("config", mcp.Description("实例配置 JSON 字符串（name, port, xlsx_file等）"), mcp.Required()),
	), toolHandler(client, func(c *SimulatorClient, args map[string]any) (any, error) {
	...
	s.AddTool(mcp.NewTool("update_instance",
		...
		mcp.WithString("instance_id", mcp.Description("实例ID"), mcp.Required()),
		mcp.WithString("config", mcp.Description("更新后的实例配置 JSON"), mcp.Required()),
	), toolHandler(client, func(c *SimulatorClient, args map[string]any) (any, error) {
...
func NewDataInterfaceServer(client *SimulatorClient) *server.MCPServer {
	...
	s.AddTool(mcp.NewTool("read_point",
		...
		mcp.WithString("instance_id", mcp.Description("实例ID"), mcp.Required()),
		mcp.WithNumber("ioa", mcp.Description("信息体地址"), mcp.Required()),
	), toolHandler(client, func(c *SimulatorClient, args map[string]any) (any, error) {
	...
	s.AddTool(mcp.NewTool("write_points",
		...
		for i, p := range pointsArg {
			...
			if !ok {
				return nil, fmt.Errorf("points[%d]: expected object, got %T", i, p)
			}
			entry := pointEntry{IOA: getUint32Arg(pMap, "ioa")}
			if entry.IOA == 0 {
				return nil, fmt.Errorf("points[%d]: missing or invalid ioa", i)
			}
	...
	s.AddTool(mcp.NewTool("import_auto_changes",
		...
		mcp.WithString("instance_id", mcp.Description("实例ID"), mcp.Required()),
		mcp.WithString("csv_content", mcp.Description("CSV 内容，包含信息体地址,测点名称,自动变化模式,A~G参数列"), mcp.Required()),
	), toolHandler(client, func(c *SimulatorClient, args map[string]any) (any, error) {
	...
	s.AddTool(mcp.NewTool("upload_csv",
		...
		mcp.WithString("instance_id", mcp.Description("实例ID"), mcp.Required()),
		mcp.WithString("csv_content", mcp.Description("CSV 文件内容"), mcp.Required()),
	), toolHandler(client, func(c *SimulatorClient, args map[string]any) (any, error) {
...
func toolHandler(client *SimulatorClient, fn func(*SimulatorClient, map[string]any) (any, error)) func(context.Context, mcp.CallToolRequest) (*mcp.CallToolResult, error) {
	return func(ctx context.Context, req mcp.CallToolRequest) (*mcp.CallToolResult, error) {
		...
		if raw, ok := result.(json.RawMessage); ok {
			...
			// Indent failed — return raw string instead of byte array
