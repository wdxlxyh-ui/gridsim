...
1. **创建实例** — 配置管理页 → "新建实例" → 填写名称、端口、选择点表文件
2. **启动实例** — 配置管理页 → 点击实例的 "启动" 按钮
3. **置数** — 实例详情页 → 找到测点 → 输入值 → 点击 "置数"
4. **配置自动变化** — 实例详情页 → 策略 Tab → 选择策略 → 设置参数
5. **CSV 回放** — 实例详情页 → CSV 回放卡片 → 上传文件 → 配置列映射
6. **趋势查看** — 趋势对比页 → 选择实例和测点 → 实时观察曲线
