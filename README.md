...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v2.3.0** | 2026-05-16 | **代码优化与Bug修复** — 前端路由守卫、API错误封装、TrendPage tooltip修复；后端recovery/logging中间件、统一错误响应格式、变化上送日志优化；修复版本号显示、构建产物结构 |
...
- **前端代码优化** — 添加路由守卫、提取 useApi composable 封装错误处理、优化 JWT 解析错误处理、修复 401 跳转使用 replaceState
- **实时趋势优化** — 请求改为 Promise.all 并行获取、修复 tooltip 鼠标悬停数值显示问题、移除调试用模拟数据
- **后端代码优化** — 添加 recovery 和 logging 中间件、统一错误响应格式 `{error, code, details}`、变化上送日志降为 debug 级别
- **修复** — 修复版本号显示 2.3.0、修复构建产物目录结构
