...
| 版本 | 日期 | 说明 |
|------|------|------|
| **v2.5.2** | 2026-05-21 | **CSV 文件下拉选择+MCP登录修复** — CSV 回放支持从已有文件下拉选择（共享目录+实例私有）；新增 `GET /csv-files` API 和 `list_csv_files` MCP 工具；修复前端登录路径不匹配；修复 MCP write_points schema 不兼容 OpenCode |
